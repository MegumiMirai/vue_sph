...
export default {
  ...
  methods: {
    ...
    handler:throttle(function(type, value, cart) {
      switch (type) {
        case 'sub':
      ...
      try {
        this.$store.dispatch('detail/addOrUpdateShopCat', {
          skuId: cart.skuId,
          skuNum: value,
        })
        this.getData()
    ...
    deleteGoods(cart) {
      try {
        this.$store.dispatch('shopcart/deleteGoods', cart.skuId)
        this.getData()
      } catch (error) {
        alert(error.message)
      }
  ...
  computed: {
    ...
    totalPrice() {
      ...
      //   }
      // })
      return this.cartInfoList.reduce((prev, cur) => {
        return prev + cur.skuNum * cur.skuPrice
      }, total)
    },
    isAllChecked() {
