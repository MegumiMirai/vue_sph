  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="user in userAddressInfo"
        :key="user.id"
      >
        <span class="username" :class="{ selected: user.isDefault == 1 }">{{
          user.consignee
        }}</span>
        <p @click="changeDefault(user, userAddressInfo)">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="order in trade.detailArrayList"
          :key="order.id"
        >
          <li>
...
export default {
  ...
  methods: {
    // 切换默认地址
    changeDefault(user, userAddressInfo) {
      // 先把所有都值为0
      userAddressInfo.forEach((userAddress) => (userAddress.isDefault = 0))
      // 再把点击的置为1
      user.isDefault = 1
    },
    async submitOrder() {
      let { consignee, phoneNum, fullAddress } = this.userAddressDefault
      let data = {
        consignee,
  ...
  computed: {
    ...mapState('trade', ['userAddressInfo', 'trade']),
    // 最终提交订单的用户信息
    userAddressDefault() {
      return (
        this.userAddressInfo.find(
